 ...
 * 验证评论内容质量
 */
export function validateCommentContent(content: string, options: ValidationOptions = {}): ValidationResult {
  ...
  // 检查连续标点符号
  const consecutivePunctuationResult = checkConsecutivePunctuation(trimmedContent, opts.maxConsecutivePunctuation);
  ...
  // 检查连续空格
  const consecutiveSpacesResult = checkConsecutiveSpaces(trimmedContent, opts.maxConsecutiveSpaces);
  if (!consecutiveSpacesResult.valid) {
    errors.push(...consecutiveSpacesResult.errors);
  }

  // 检查连续换行
  const consecutiveNewlinesResult = checkConsecutiveNewlines(trimmedContent, opts.maxConsecutiveNewlines);
 ...
 * 检查连续标点符号
 */
function checkConsecutivePunctuation(content: string, maxConsecutive: number): ValidationResult {
  const errors: string[] = [];
  ...
  for (const punct of punctuations) {
    ...
    if (matches) {
      errors.push(`Too many consecutive "${punct}" (max ${maxConsecutive}). Found: ${matches[0]}`);
    }
  }
  ...
  if (mixedMatches) {
    errors.push(`Too many consecutive punctuation marks (max ${maxConsecutive}). Found: ${mixedMatches[0]}`);
  }
 ...
 * 检查连续空格
 */
function checkConsecutiveSpaces(content: string, maxConsecutive: number): ValidationResult {
 ...
 * 检查连续换行
 */
function checkConsecutiveNewlines(content: string, maxConsecutive: number): ValidationResult {
 ...
 * 检查阻止模式
 */
function checkBlockPatterns(content: string, patterns: RegExp[]): ValidationResult {
  const errors: string[] = [];
 ...
 * 清理内容（自动修复一些问题）
 */
export function sanitizeContent(content: string, options: ValidationOptions = {}): string {
  const opts = { ...DEFAULT_OPTIONS, ...options };
  let cleaned = content;
